...
fn test_manual_cte_query() {
    ...
    executor.execute_sql("CREATE TABLE __cte_nums (a INT64, b INT64)").unwrap();
    executor.execute_sql("INSERT INTO __cte_nums VALUES (1, 2)").unwrap();

    // Execute the same query
    let result = executor.execute_sql("SELECT a + 1, b + 1 FROM __cte_nums WHERE a < 5").unwrap();
